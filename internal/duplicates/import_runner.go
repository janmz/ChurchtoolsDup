...
func (r ImportRunner) Run(groups [][]csvfile.DupEntry, opts ImportOptions) ([]ImportResult, error) {
	...
	for i, group := range groups {
		...
		if opts.DryRun {
			...
			result.Message = formatDryRunMessage(primary, others, alreadyLinked, r.groupName())
			results = append(results, result)
			continue
		}
...
func formatDryRunMessage(primary int, others, alreadyLinked []int, groupName string) string {
	...
	case len(alreadyLinked) > 0 && len(toLink) == 0:
		return fmt.Sprintf(
			"Dry-Run: Person %d ist bereits mit %s verknüpft; Gruppe %q prüfen",
			primary,
	...
	case len(alreadyLinked) > 0:
		return fmt.Sprintf(
			"Dry-Run: Person %d mit %s verknüpfen (%s bereits verknüpft), Gruppe %q",
			primary,
			formatIDList(toLink),
			formatIDList(alreadyLinked),
			groupName,
	...
	default:
		return fmt.Sprintf(
			"Dry-Run: würde Person %d mit %s verknüpfen und in Gruppe %q aufnehmen",
			primary,
			formatIDList(toLink),
			groupName,
		)
